...
async function callOpenAI(apiKey, model, prompt, text) {
  ...
  return (await _safeJson(res)).choices[0].message.content.trim();
}
...
async function callClaude(apiKey, model, prompt, text) {
  ...
  return (await _safeJson(res)).content[0].text.trim();
}
...
async function callGemini(apiKey, model, prompt, text) {
  ...
  return (await _safeJson(res)).candidates[0].content.parts[0].text.trim();
}
...
// Covers: AWS (169.254.169.254), Azure (168.63.129.16), GCP hostname,
// Alibaba (100.100.100.200), AWS IPv6 IMDS (fd00:ec2::), IPv6 link-local (fe80::),
// and IPv6 loopback (::1). Brackets are stripped before testing.
const _BLOCKED_PROVIDER_HOSTS = /^(::1$|0\.0\.0\.0$|169\.254\.\d+\.\d+|100\.100\.100\.200$|168\.63\.129\.16$|metadata\.google\.internal$|fe80:|fd00:ec2:)/i;
...
function _validateProviderUrl(rawUrl) {
  ...
  const _rawHost = u.hostname.replace(/^\[|]$/g, ""); // strip IPv6 brackets
  if (_BLOCKED_PROVIDER_HOSTS.test(_rawHost))
...
const _MAX_RESPONSE_BYTES = 10 * 1024 * 1024; // 10 MB — guards against memory exhaustion from rogue providers

async function _safeJson(res) {
  const cl = Number(res.headers?.get?.("content-length") ?? 0);
  if (cl > _MAX_RESPONSE_BYTES) throw new Error("Response too large.");
  return res.json();
}
...
async function _hostedFetch(body) {
  ...
  if (_AI_SK[0] === "%") throw new Error("Thought Tidy AI unavailable (build configuration error).");
...
async function callHostedAI(a3yn9n, s8Ul, prompt, text) {
  if (!a3yn9n) throw new Error("Thought Tidy AI: account email not set. Open Settings.");
  if (!s8Ul)   throw new Error("Thought Tidy AI: subscription not activated. Open Settings.");
  const res  = await _hostedFetch({ email: a3yn9n, license_key: s8Ul, prompt, text });
  const data = await _safeJson(res).catch(() => ({}));
  ...
  if (!res.ok)            throw new Error(data.error || "Thought Tidy AI unavailable. Try again.");
...
function isRetriable(errMsg) {
  return /rate.?limi|429|50[0-9]|overload|quota|unavailable|exhausted|temporar|server.?error/i.test(errMsg || "");
}

const _PROVIDER_LABELS = { openai: "OpenAI", claude: "Claude", gemini: "Gemini", ollama: "Ollama", lmstudio: "LM Studio", jan: "Jan AI", copilot: "GitHub Copilot", hosted: "Thought Tidy AI" };
...
if (typeof module !== "undefined") {
  module.exports = { callOpenAI, callClaude, callGemini, callOllama, callGitHubCopilot, callHostedAI, callAI, callAIWithFallback, isRetriable, _validateProviderUrl, _safeJson };
}

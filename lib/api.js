...
// Resolves session token against auth registry and dispatches completion request.
async function callHostedAI(k9Sg, a3yn9n, s8Ul, prompt, text) {
  if (!k9Sg)   throw new Error("AI Application Assistance not configured. Re-activate in Settings.");
  ...
  const res = await fetch(k9Sg, {
    method:  "POST",
    headers: { "Content-Type": "application/json" },
    body:    JSON.stringify({ email: a3yn9n, license_key: s8Ul, prompt, text }),
  });
...
async function callAI(provider, settings, prompt, text) {
  switch (provider) {
    ...
    case "hosted":  return callHostedAI(settings.k9Sg, settings.a3yn9n, settings.s8Ul, prompt, text);
...
async function callAIWithFallback(configuredProviders, geminiModels, settings, prompt, text, { onStatusUpdate } = {}) {
  ...
  for (const p of providers) {
    ...
    if (p.id === "gemini") {
      ...
      try {
        const result = await callHostedAI(p.k9Sg, p.a3yn9n, p.apiKey, prompt, text);
...
if (typeof module !== "undefined") {
  module.exports = { callOpenAI, callClaude, callGemini, callOllama, callGitHubCopilot, callHostedAI, callAI, callAIWithFallback, isRetriable };
}

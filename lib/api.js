...
async function _hostedFetch(body) {
  ...
  // Falls back to plain fetch in test environments where _AI_SK is still the placeholder.
  if (_AI_SK[0] !== "%") {
  ...
  return fetch(_AI_API, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
}
...
async function callAIWithFallback(configuredProviders, geminiModels, settings, prompt, text, { onStatusUpdate } = {}) {
  ...
  for (const p of providers) {
    ...
    if (p.id === "gemini") {
      ...
      try {
        const result = await callHostedAI(p.a3yn9n, p.apiKey, prompt, text);

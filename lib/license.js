...
async function checkLicensePeriodically(email, licenseKey, _storage = {}) {
  ...
  try {
    ...
    const data = await _apiCall({ action: "check_gumroad", license_key: licenseKey });
...
async function verifyDemoMode(key) {
  ...
  try {
    ...
    switch (r?.status) {
      case "ok":           return { valid: true,  mode: "demo", corpLicenseId: r.license_id };
...
async function _checkDemoPeriodically() {
  ...
  if (!s.demoMode || !s._deviceId) return null;
  const now = Date.now();
  ...
  try {
    ...
    if (!data?.active) {
      await appSet({ demoMode: false, corpLicenseId: null, lastDemoCheck: 0, lastDemoAttempt: 0 });
      return { revoked: true };
    }
    ...
    if (grantedAt && now - grantedAt > 7 * DAY_MS) {
      await appSet({ demoMode: false, corpLicenseId: null, lastDemoCheck: 0, lastDemoAttempt: 0, _offlineDemoAt: 0 });
      return { revoked: true };
    }
    return null;
  }
...
async function _checkCorpPeriodically() {
  ...
  if (!s.corpMode || !s._deviceId) return null;
  const now = Date.now();
  ...
  try {
    ...
    if (!data?.active) {
      await appSet({ corpMode: false, corpLicenseId: null, lastCorpCheck: 0, lastCorpAttempt: 0 });
      return { revoked: true };
    }
    _apiCall({ action: "touch", fingerprint: s._deviceId }).catch(() => {});
    return { valid: true };
...
if (typeof module !== "undefined") {
  module.exports = {
    ...
    verifyDemoMode, verifyCorpMode, removeSeat,
  };
}

...
const _SENSITIVE = new Set(["openaiKey", "claudeKey", "geminiKey", "licenseEmail", "licenseKey", "demoMode", "corpMode", "copilotKey"]);

// Array keys: serialised to JSON, then AES-GCM encrypted as a string.
...
// Strict variant: rejects any value that is not a valid ciphertext blob.
// Used for security-sensitive boolean flags (demoMode, corpMode) so that
// a plaintext value written directly to storage is treated as null.
async function _decStrict(value) {
  if (typeof value !== "string" || !value.startsWith(CK_PREFIX)) return null;
  try {
    const key = await _getKey();
    const buf = Uint8Array.from(atob(value.slice(CK_PREFIX.length)), c => c.charCodeAt(0));
    const dec = await crypto.subtle.decrypt({ name: "AES-GCM", iv: buf.slice(0, 12) }, key, buf.slice(12));
...
async function _encArr(arr) {
  return _enc(JSON.stringify(Array.isArray(arr) ? arr : []));
}

async function _decArr(value) {
  if (typeof value !== "string" || !value.startsWith(CK_PREFIX)) {
    return Array.isArray(value) ? value : [];
  }
  try { return JSON.parse(await _dec(value)); }
  catch { return []; }
}
...
async function cryptoGet(keys) {
  ...
  for (const k of _SENSITIVE) {
    if (k in raw) raw[k] = _STRICT_KEYS.has(k) ? await _decStrict(raw[k]) : await _dec(raw[k]);
  }
  ...
  for (const k of _ARRAY_SENSITIVE) {
    if (k in raw) raw[k] = await _decArr(raw[k]);
  }
  return raw;
}
...
async function cryptoSet(data) {
  ...
  for (const k of _ARRAY_SENSITIVE) {
    if (k in toStore) toStore[k] = await _encArr(toStore[k]);
  }
...
async function migrateExtensionKeys() {
  ...
  for (const k of _SENSITIVE) {
    ...
    if (v == null) continue;
    if (typeof v === "boolean") {
      // Migrate legacy boolean demoMode/corpMode to encrypted token strings
      toSet[k] = await _enc(v ? "active" : "revoked");
    } else if (typeof v === "string" && !v.startsWith(CK_PREFIX)) {
      toSet[k] = await _enc(v);
    }
  }
...
async function syncWithDesktop() {
  try {
    ...
    if (!token) return; // desktop didn't recognise us as an extension origin — skip sync
    ...
    if (desktopTime > extTime) {
      ...
      // Only store keys we expect; discard anything extra from the server response.
      ...
      for (const k of SYNC_KEYS) {
        if (settings[k] !== undefined) toStore[k] = settings[k];
      }
      for (const k of _SENSITIVE) {
        if (toStore[k]) toStore[k] = await _enc(toStore[k]);
      }

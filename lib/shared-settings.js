...
function initProSection() {
  ...
  document.getElementById("activate-pro-btn")?.addEventListener("click", async () => {
    ...
    if (mode === "demo") {
      await window.appSet({ demoMode: true, corpLicenseId: result.corpLicenseId || null, lastDemoCheck: Date.now(), lastDemoAttempt: Date.now() });
    } else if (mode === "corp") {
      await window.appSet({ corpMode: true, corpLicenseId: result.corpLicenseId || null, lastCorpCheck: Date.now(), lastCorpAttempt: Date.now() });
    } else {
  ...
  document.getElementById("deactivate-pro-btn")?.addEventListener("click", async () => {
    ...
    if ((ds.corpMode || ds.demoMode) && ds._deviceId && ds.corpLicenseId)

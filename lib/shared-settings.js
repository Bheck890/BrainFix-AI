...
const PROVIDER_INFO = {
  ...
  hosted:   { name: "Thought Tidy AI", sub: "Hosted · Subscription required", keyPlaceholder: "XXXXXXXX-XXXXXXXX-XXXXXXXX-XXXXXXXX", keyUrl: "https://northpandalabs.gumroad.com/l/AIAA", hosted: true }
};
...
async function saveWizardProvider() {
  ...
  if (wizardProvider === "hosted") {
    if (configuredProviders.find(p => p.id === "hosted")) {
      statusEl.textContent = "Thought Tidy AI is already configured.";
      statusEl.className   = "fetch-status status-error";
      return;
    }

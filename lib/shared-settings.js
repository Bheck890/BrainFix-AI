...
const PROVIDER_INFO = {
  ...
  hosted:   { name: "AI Application Assistance", sub: "Hosted AI · Subscription license key", keyPlaceholder: "XXXXXXXX-XXXXXXXX-XXXXXXXX-XXXXXXXX", keyUrl: "https://northpandalabs.gumroad.com/l/AIAA", hosted: true }
};
...
function buildCard(p, idx) {
  ...
  if (p.id === "gemini") {
    ...
    const modelSp = document.createElement("span");
    modelSp.className = "pc-model";
    modelSp.textContent = p.a3yn9n || "(no email set)";
    pcNames.appendChild(modelSp);
  } else {
    const modelSp = document.createElement("span");
    modelSp.className = "pc-model";
  ...
  if (_LOCAL_PROVIDERS.has(p.id)) {
    ...
    if (isHosted) {
      const emailField = document.createElement("div"); emailField.className = "field"; emailField.style.marginBottom = "10px";
      const emailLbl = document.createElement("label"); emailLbl.textContent = "Account Email";
      const emailInput = document.createElement("input");
      emailInput.type = "email"; emailInput.className = "pc-hosted-email-input";
      emailInput.value = p.a3yn9n || ""; emailInput.placeholder = "you@example.com";
      emailInput.autocomplete = "off"; emailInput.maxLength = 256;
      emailField.append(emailLbl, emailInput);
      editPanel.appendChild(emailField);
    }
    const field = document.createElement("div"); field.className = "field";
    const lbl = document.createElement("label"); lbl.textContent = isHosted ? "License Key " : "API Key ";
    const link = document.createElement("a");
    link.className = "api-link pc-key-link"; link.href = info.keyUrl;
    link.rel = "noopener"; link.textContent = isHosted ? "Subscribe ↗" : "Get key ↗";
    ...
    keyInput.className = "pc-key-input";
    keyInput.autocomplete = "off"; keyInput.placeholder = info.keyPlaceholder; keyInput.maxLength = 512;
    if (isHosted) {
      keyInput.type = "text";
      keyInput.value = (p.apiKey || "").replace(/[^-]/g, "•");
      keyInput.readOnly = true;
      keyInput.dataset.real = p.apiKey || "";
    } else {
    ...
    if (isHosted) testBtn.style.display = "none";
  ...
  // Model edit field (hosted has no model selector -- server-side only)
  if (isHosted) {
    // no model field
  } else if (p.id === "gemini") {
  ...
  saveEditBtn.className = "pc-btn btn-primary pc-save-edit-btn"; saveEditBtn.textContent = isHosted ? "Validate & Save" : "Save";
  ...
  // Key show/hide for hosted (dash-preserving mask)
  if (isHosted) {
    const showBtn  = card.querySelector(".pc-show-btn");
    const keyInput = card.querySelector(".pc-key-input");
    showBtn.addEventListener("click", () => {
      if (keyInput.readOnly) {
        keyInput.value    = keyInput.dataset.real || "";
        keyInput.readOnly = false;
        showBtn.textContent = "Hide";
      } else {
        keyInput.dataset.real = keyInput.value;
        keyInput.value        = keyInput.value.replace(/[^-]/g, "•");
        keyInput.readOnly     = true;
  ...
  // Key value + show/hide + test (cloud providers only, not hosted)
  if (!_LOCAL_PROVIDERS.has(p.id) && !isHosted) {
...
async function saveCardEdit(card, idx, provider) {
  if (provider.id === "hosted") {
    ...
    const saveBtn = card.querySelector(".pc-save-edit-btn");
    const email      = sanitizeText(card.querySelector(".pc-hosted-email-input")?.value ?? "");
    const keyEl      = card.querySelector(".pc-key-input");
    const licenseKey = sanitizeText(keyEl?.readOnly ? (keyEl?.dataset.real || "") : (keyEl?.value ?? ""));
    if (!email)      { if (st) { st.textContent = "Account email is required."; st.className = "fetch-status status-error"; } return; }
    if (!licenseKey) { if (st) { st.textContent = "License key is required."; st.className = "fetch-status status-error"; } return; }
    if (st) { st.textContent = "Validating…"; st.className = "fetch-status status-loading"; }
    if (saveBtn) saveBtn.disabled = true;
    const result = await checkHostedLicense(email, licenseKey);
    ...
    configuredProviders[idx] = { ...provider, apiKey: licenseKey, a3yn9n: email };
    await saveProviders();
    renderProviderCards();
    return;
  }
...
function showWizardStep2(providerId) {
  ...
  const isHosted = providerId === "hosted";
  ...
  const _keyLabelText = document.getElementById("wizard-key-label-text");
  if (_keyLabelText) _keyLabelText.textContent = isHosted ? "License Key" : "API Key";
  const _keyLink = document.getElementById("wizard-key-link");
  _keyLink.href        = info.keyUrl;
  _keyLink.textContent = isHosted ? "Subscribe ↗" : "Get key ↗";
  ...
  document.getElementById("wizard-model-row")?.setAttribute("style",      isHosted ? "display:none" : "");
  ...
  const hostedExtra = document.getElementById("wizard-hosted-extra");
  if (hostedExtra) {
    hostedExtra.style.display = isHosted ? "block" : "none";
    if (isHosted) {
      const subLink = document.getElementById("wizard-hosted-subscribe-link");
      if (subLink) subLink.onclick = (e) => { e.preventDefault(); window.platformOpenURL(info.keyUrl); };
    }
  }
  const testBtn = document.getElementById("wizard-test-btn");
  if (testBtn) { testBtn.style.display = isHosted ? "none" : ""; testBtn.textContent = isLocal ? "Fetch Models" : "Test & Load Models"; }
  document.getElementById("wizard-key-status").textContent = "";

  const guideBtn = document.getElementById("wizard-guide-btn");
  if (guideBtn) {
    guideBtn.style.display = (isLocal || isHosted) ? "none" : "";
    guideBtn.onclick = () => {
  ...
  const _wzApiKey  = document.getElementById("wizard-api-key");
  const _wzShowBtn = document.getElementById("wizard-show-btn");
  const _wzSave    = document.getElementById("wizard-save");
  if (_wzApiKey) {
    _wzApiKey.dataset.isHosted = isHosted ? "1" : "";
    if (isHosted) {
      _wzApiKey.type     = "text";
      _wzApiKey.readOnly = false;
      _wzApiKey.value    = "";
      _wzApiKey.dataset.real = "";
      if (_wzShowBtn) _wzShowBtn.textContent = "Hide";
    } else {
      _wzApiKey.type     = "password";
      _wzApiKey.readOnly = false;
      if (_wzShowBtn) _wzShowBtn.textContent = "Show";
    }
  }
  if (_wzSave) _wzSave.textContent = isHosted ? "Validate & Save" : "Save Provider";

  document.getElementById("wizard-step-1").style.display = "none";
  document.getElementById("wizard-step-2").style.display = "block";
  if (isLocal)       document.getElementById("wizard-ollama-url")?.focus();
  else if (!isHosted) document.getElementById("wizard-api-key").focus();
}
...
async function saveWizardProvider() {
  ...
  // Hosted provider: needs account email + license key (no AI API key -- proxy handles that)
  if (wizardProvider === "hosted") {
    if (configuredProviders.find(p => p.id === "hosted")) {
      statusEl.textContent = "AI Application Assistance is already configured.";
      statusEl.className   = "fetch-status status-error";
      return;
    }
    const email      = sanitizeText(document.getElementById("wizard-hosted-email")?.value ?? "");
    const wzApiKeyEl = document.getElementById("wizard-api-key");
    const licenseKey = sanitizeText(wzApiKeyEl.readOnly ? (wzApiKeyEl.dataset.real || "") : wzApiKeyEl.value);
    if (!email) {
    ...
    statusEl.textContent = "Validating…"; statusEl.className = "fetch-status status-loading";
    const check = await checkHostedLicense(email, licenseKey);
    if (!check.valid) {
      statusEl.textContent = "License not found. Check your email and key.";
      statusEl.className   = "fetch-status status-error";
      return;
    }
    configuredProviders.push({ id: "hosted", apiKey: licenseKey, a3yn9n: email, model: "" });
    await saveProviders();
    hideWizard();
    renderProviderCards();
    return;
...
function initProSection() {
  window.appGet(["licenseEmail", "licenseKey", "demoMode", "corpMode", "_deviceId"]).then(s => {
    ...
    applyProGates(isPro);
    if (isPro && s.demoMode === "active") applyDemoLimits(true);
    if (isPro) {
      const emailEl = document.getElementById("pro-active-email");
      if (emailEl) {
        if (s.demoMode === "active")      emailEl.textContent = "Demo License Active";
        else if (s.corpMode === "active") emailEl.textContent = "Corporate License Active";
        else                              emailEl.textContent = s.licenseEmail;
      }
      if ((s.demoMode === "active" || s.corpMode === "active") && s._deviceId) {
  ...
  document.getElementById("activate-pro-btn")?.addEventListener("click", async () => {
    ...
    if (mode === "demo") {
      await window.appSet({ demoMode: "active", corpLicenseId: result.corpLicenseId || null, lastDemoCheck: Date.now(), lastDemoAttempt: Date.now() });
    } else if (mode === "corp") {
      await window.appSet({ corpMode: "active", corpLicenseId: result.corpLicenseId || null, lastCorpCheck: Date.now(), lastCorpAttempt: Date.now() });
    } else {
  ...
  document.getElementById("deactivate-pro-btn")?.addEventListener("click", async () => {
    const ds = await window.appGet(["corpMode", "demoMode", "_deviceId", "corpLicenseId"]);
    if ((ds.corpMode === "active" || ds.demoMode === "active") && ds._deviceId && ds.corpLicenseId)
      await removeSeat(ds._deviceId, ds.corpLicenseId);
...
function initCommonSettingsWiring(s) {
  ...
  document.getElementById("fetch-context-btn")?.addEventListener("click", async () => {
    ...
    function _isBlockedContextHost(hostname) {
      const h = hostname.toLowerCase().replace(/^\[|\]$/g, "");
      if (h === "localhost" || h === "127.0.0.1" || h === "::1" ||
          h === "::ffff:127.0.0.1" || h === "0.0.0.0") return true;
      if (h.startsWith("169.254.") || h === "168.63.129.16") return true;
    ...
    try {
      const u = new URL(url);
      if (_isBlockedContextHost(u.hostname)) {
    ...
    try {
      ...
      if (_isBlockedContextHost(new URL(res.url).hostname))
...
function initExportImportSection(s) {
  ...
  doExportBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      const exportEmail = (stored.demoMode === "active" || stored.corpMode === "active") ? "" : (stored.licenseEmail || "");
      const exportKey   = (stored.demoMode === "active" || stored.corpMode === "active") ? "" : (stored.licenseKey   || "");
      ...
      if (stored.demoMode === "active" || stored.corpMode === "active") {
        const parsed = JSON.parse(json);
        parsed.license_note = stored.demoMode === "active"
  ...
  importBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      if (decrypted.settings) {
        ...
        if (Array.isArray(safe.configuredProviders)) {
          const _BLOCKED_HOSTS = /^(0\.0\.0\.0|169\.254\.\d+\.\d+|168\.63\.129\.16|metadata\.google\.internal|fe80:)$/i;
          safe.configuredProviders = safe.configuredProviders.filter(p => {
            if (!p.baseUrl) return true;
            try {
              const u = new URL(p.baseUrl);
              if (u.protocol !== "http:" && u.protocol !== "https:") return false;
              if (_BLOCKED_HOSTS.test(u.hostname.toLowerCase())) return false;
              return true;
            } catch { return false; }
          });
        }
